...
var _ = Describe("Smart playlist criteria SQL", func() {
	...
	DescribeTable("expressions",
		...
		Entry("gte safe (value >= default)", criteria.Gte{"playCount": 0}, "(annotation.play_count >= ? OR annotation.play_count IS NULL)", 0),
		Entry("gte unsafe (value > default)", criteria.Gte{"playCount": 5}, "annotation.play_count >= ?", 5),
		Entry("lte safe (value <= default)", criteria.Lte{"playCount": 0}, "(annotation.play_count <= ? OR annotation.play_count IS NULL)", 0),
		Entry("lte unsafe (value < default)", criteria.Lte{"playCount": -1}, "annotation.play_count <= ?", -1),
